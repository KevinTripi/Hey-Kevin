...
# This function is used to run gpt for extracting object name
...
# Explaining chatgpt how to spit out humorous comments
...
# This function runs gpt to generate humorous comments and returns it as a json with 3 key value pairs
...
# __main__
with open('../bing_exports/image-128_export.json', 'r') as file:
    bing_json = json.loads(file.read())

db = FirebaseRestAPI(HEY_KEVIN_URL)

# This function runs the pipeline: bing json -> extract object name using gpt -> generate gpt comments and post to firebase
def run_pipeline(bingJSON):
    object_name_json = get_object_name(bingJSON)
    if "error" in object_name_json:
        print(object_name_json)
        return
    comment_json = get_gpt_comments(object_name_json["object_name"])
    if comment_json:
